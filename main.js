...
class Player {
    ...
    touch(x, y) {
        for (let i = 0; i < this.hold.block.length; i++) {
            for (let j = 0; j < this.hold.block[i].length; j++) {
                if (this.hold.block[i][j] !== 0 && map[y + i][x + j] !== 0 && y !== 0 ) {
                    return true;
                }
            }
        }
    ...
    draw() {
        ...
        for (let i = 0; i < this.hold.block.length; i++) {
            for (let j = 0; j < this.hold.block[i].length; j++) {
                if (this.hold.block[i][j] === 0 || (map[this.y+i][this.x+j]===8) ) continue;
        ...
        for (let i = 0; i < this.next.block.length; i++) {
            for (let j = 0; j < this.next.block[i].length; j++) {
                decoration(15 + j, 2 + i, colortypes[this.next.block[i][j]]);
            }
        }
        ctx.save();
        ...
        ctx.shadowBlur = 10;
        ctx.font = "40px arial";
        ctx.fillText("next", 15 * screen.span, 1 * screen.span);
        ctx.fillText("keep", 15 * screen.span, 16 * screen.span);
        ctx.fillText("score", 15 * screen.span, 10 * screen.span);
        ctx.fillText(this.score, 15 * screen.span, 12 * screen.span);
        ctx.fillText("bombs",15 * screen.span, 6 * screen.span)
        ctx.fillText(bombs,15 * screen.span, 8 * screen.span)
